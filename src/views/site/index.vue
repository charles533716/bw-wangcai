      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-s-home"
            @click="handleEnterSiteAdmin(scope.row)"
          >进入站点后台</el-button>
          <el-button
            size="mini"
            type="text"
...
import { resolvePrototypePath, setBackendContext } from '@/utils/prototypeBackend'
...
export default {
  ...
  methods: {
    ...
    handleEnterSiteAdmin(row) {
      const siteCode = row.code || '2222'
      setBackendContext('site', { siteCode })
      this.$message.success(`已进入站点后台：${siteCode}`)
      window.location.href = resolvePrototypePath('/site-admin/index')
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
