<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="100px">
     ...
     <!-- <el-form-item label="站点编码" prop="siteCode">
      ...
      <!-- 场馆名称 -->
      <el-form-item label="场馆名称" prop="venueCode" v-hasPermi="['venue:list']">
        <el-select
  ...
  <!--  <el-row :gutter="10" class="mb8">
    ...
    <el-table v-loading="loading" :data="betRecordsList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" align="center" prop="id" width="80" />
      <el-table-column label="注单号" align="center" prop="generatedId" width="180" show-overflow-tooltip />
...
<el-table-column label="上级代理" align="center" prop="agentCode" width="120" />
   <el-table-column label="场馆类型" align="center" prop="venueType" width="120" />
      <el-table-column label="场馆名称" align="center" prop="venueName" width="120" />
        <el-table-column label="游戏ID" align="center" prop="gameTypeId" width="150" show-overflow-tooltip />
      <el-table-column label="游戏名称" align="center" prop="gameTypeName" width="150" show-overflow-tooltip />
     <el-table-column label="下注详情" align="center" prop="betContent" width="120" />
      ...
      <el-table-column label="净盈利" align="center" prop="netAmount" width="100">
        <template slot-scope="scope">
          <span :class="{'text-success': scope.row.netAmount > 0, 'text-danger': scope.row.netAmount < 0}">
      ...
      <el-table-column label="订单状态" align="center" prop="obBetStatus" width="100">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.obBetStatus === 1 ? 'success' : scope.row.obBetStatus === 2 ? 'warning' : 'info'"
          >
            {{ scope.row.obBetStatus === 1 ? '已结算' : scope.row.obBetStatus === 2 ? '不结算' : '未结算' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="结算时间" align="center" prop="netAt" width="160">
        <template slot-scope="scope">
          <span>{{ scope.row.netAt ? parseTime(scope.row.netAt, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
        </template>
      </el-table-column>
   ...
   <!-- <el-dialog title="同步注单数据" :visible.sync="syncOpen" width="500px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitSyncForm" :loading="syncLoading">开始同步</el-button>
    ...
    <el-dialog title="注单详情" :visible.sync="viewOpen" width="800px" append-to-body>
      <el-descriptions :column="2" border v-if="currentRecord">
        ...
        <el-descriptions-item label="三方注单号">{{ currentRecord.generatedId }}</el-descriptions-item>
        <el-descriptions-item label="会员账号">{{ currentRecord.userName }}</el-descriptions-item>
        <el-descriptions-item label="三方游戏账号">{{ currentRecord.playerName }}</el-descriptions-item>
        <el-descriptions-item label="场馆名称">{{ currentRecord.venueName }}</el-descriptions-item>
        <el-descriptions-item label="游戏名称">{{ currentRecord.gameTypeName }}</el-descriptions-item>
        <el-descriptions-item label="平台名称">{{ currentRecord.platformName }}</el-descriptions-item>
        <el-descriptions-item label="下注金额">{{ currentRecord.betAmount ? currentRecord.betAmount.toFixed(2) : '0.00' }}</el-descriptions-item>
        <el-descriptions-item label="有效投注">{{ currentRecord.validBetAmount ? currentRecord.validBetAmount.toFixed(2) : '0.00' }}</el-descriptions-item>
        <el-descriptions-item label="净盈利">{{ currentRecord.netAmount ? currentRecord.netAmount.toFixed(2) : '0.00' }}</el-descriptions-item>
        <el-descriptions-item label="派奖金额">{{ currentRecord.payAmount ? currentRecord.payAmount.toFixed(2) : '0.00' }}</el-descriptions-item>
        <el-descriptions-item label="订单状态">
          <el-tag :type="currentRecord.obBetStatus === 1 ? 'success' : currentRecord.obBetStatus === 2 ? 'warning' : 'info'">
            {{ currentRecord.obBetStatus === 1 ? '已结算' : currentRecord.obBetStatus === 2 ? '不结算' : '未结算' }}
          </el-tag>
        ...
        <el-descriptions-item label="下注时间">{{ parseTime(currentRecord.betAt) }}</el-descriptions-item>
        <el-descriptions-item label="结算时间">{{ currentRecord.netAt ? parseTime(currentRecord.netAt) : '-' }}</el-descriptions-item>
        <el-descriptions-item label="同步时间">{{ parseTime(currentRecord.syncAt) }}</el-descriptions-item>
        <el-descriptions-item label="投注内容" :span="2">{{ currentRecord.betContent }}</el-descriptions-item>
        <el-descriptions-item label="判奖结果" :span="2">{{ currentRecord.judgeResult }}</el-descriptions-item>
        <el-descriptions-item label="赛事详情" :span="2">{{ currentRecord.matchDetail }}</el-descriptions-item>
      </el-descriptions>
      <div slot="footer" class="dialog-footer">
...
<script>
import { listBetRecords, delBetRecords, syncBetRecords, getBetRecords } from "@/api/game/betRecords";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listBetRecords(params).then(response => {
    ...
    handleExport() {
      const params = this.buildQueryParams();
      this.download('/bet/records/export', {
        ...params
