            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="后台登录账号" prop="account">
                  <el-input v-model="applyForm.account" placeholder="请输入邮箱地址" maxlength="255"   @blur="handleAccountBlur"/>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
        ...
        <el-dialog title="修改申请" :visible.sync="updateOpen" width="600px" append-to-body>
          <el-form ref="updateForm" :model="updateForm" :rules="updateRules" label-width="120px">
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="后台登录账号" prop="account">
                  <el-input v-model="updateForm.account" placeholder="请输入邮箱地址" maxlength="255"   @blur="handleUpdateAccountBlur"/>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
    ...
    const validateConfirmPassword = (rule, value, callback) => {
     ...
     // 站点编码唯一性验证
        ...
        const validateSiteAccount = (rule, value, callback) => {
          ...
          // 检查站点账号是否唯一
          checkSiteAccountUnique({ account: value }).then(response => {
            if (!response.data) {
    ...
    return {
      ...
      applyRules: {
       code: [
        ...
        account: [
                  { required: true, message: "后台登录账号不能为空", trigger: "blur" },
  ...
  methods: {
    ...
    handleApply() {
      ...
      /** 站点编码失焦校验 */
        ...
        /** 站点账号失焦校验 */
        handleAccountBlur() {
          if (this.applyForm.account) {
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (emailRegex.test(this.applyForm.account)) {
              this.checkAccountUnique(this.applyForm.account, null);
            }
          }
        },
        ...
        handleUpdateAccountBlur() {
          if (this.updateForm.account) {
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (emailRegex.test(this.updateForm.account)) {
              this.checkAccountUnique(this.updateForm.account, this.updateForm.id);
            }
          }
        },
    ...
    /** 提交申请 */
       ...
       validateBeforeSubmit() {
         return new Promise((resolve, reject) => {
           ...
           const accountCheck = checkSiteAccountUnique({ account: this.applyForm.account });
