  ...
  <div class="app-container venue-agent-fee-page">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" class="filter-panel">
      ...
      <el-form-item>
        ...
        >
          <el-option v-if="canSelectSite" label="所有站点" value="" />
          <el-option
...
import {
  ...
  getVenueAgentFeeDetailTotal,
  listVenueAgentFeeDetail
} from '@/api/agent/venueAgentFeeDetail'

export default {
  name: 'SiteVenueAgentFeeDetail',
  data() {
  ...
  computed: {
    ...
    isSuperAdmin() {
      return Number(this.$store.getters.id) === 1 || this.hasRole('admin')
    },
    canSelectSite() {
  ...
  created() {
    if (!this.canSelectSite && this.userSiteCode) {
  ...
  methods: {
    hasRole(roleKey) {
      const expected = String(roleKey || '').toLowerCase()
      return this.roles.some(role => {
        if (role && typeof role === 'object') {
          return String(role.roleKey || role.key || '').toLowerCase() === expected
        }
    ...
    getList(refreshTotal = false) {
      ...
      this.loading = true
      const requests = [listVenueAgentFeeDetail(params)]
      if (refreshTotal) {
        const totalParams = { ...params }
        delete totalParams.pageNum
        delete totalParams.pageSize
        requests.push(getVenueAgentFeeDetailTotal(totalParams))
      }
      Promise.all(requests).then(([listResponse, totalResponse]) => {
        this.rows = (listResponse && listResponse.rows) || []
        if (refreshTotal) {
          this.total = Number((totalResponse && totalResponse.data) || 0)
        }
      }).finally(() => {
        this.loading = false
      })
    ...
    buildQueryParams(includePage = true) {
      ...
      const params = {
        startDate,
        endDate,
        siteCode: this.canSelectSite ? (this.queryParams.siteCode || undefined) : (this.userSiteCode || undefined),
