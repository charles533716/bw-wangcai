<template>
  <div v-loading="loading" class="reversal-report-page app-container">
    ...
    <section class="note-card note-card--dark">
      ...
      <div class="note-card__line">3. 欠款人数 / 欠款额度：以当前仍有剩余未归还金额的台账为准；代理视角下会额外并入本人向上级代理或站点借入且未还清的记录。</div>
      <div class="note-card__line">4. 当前版本的垫付统计仍以代理台账为主；仅当前代理本人借入的站点兜底台账会并入欠款展示，下级代理向站点借入的记录不在此页展示。</div>
    </section>
  </div>
</template>
...
import { getReversalStatsSummary, getReversalStatsTotal, listReversalStats } from '@/api/agent/reversalReport'
...
export default {
  ...
  methods: {
    ...
    async getList(refreshSummary = false, refreshTotal = false) {
      this.syncDateRange()
      this.loading = true
      try {
        const requests = [listReversalStats(this.queryParams)]
        if (refreshSummary) {
          requests.push(getReversalStatsSummary(this.queryParams))
        }
        if (refreshTotal) {
          requests.push(getReversalStatsTotal(this.queryParams))
        }
        ...
        let resultIndex = 1
        if (refreshSummary) {
          const summaryRes = results[resultIndex++]
          this.summary = Object.assign(createSummary(), (summaryRes && summaryRes.data) || {})
        }
        if (refreshTotal) {
          const totalRes = results[resultIndex]
          this.total = Number((totalRes && totalRes.data) || 0)
        }
      } finally {
        this.loading = false
      }
    ...
    handlePagination({ limit } = {}) {
      ...
      this.lastPageSize = this.queryParams.pageSize
      this.getList(false, refreshTotal)
    },
    handleReset() {
      this.dateRange = getRecentSevenDaysRange()
      const siteCode = !this.isSuperAdmin && this.userSiteCode ? this.userSiteCode : ''
      this.queryParams = {
        pageNum: 1,
        pageSize: 10,
        startDate: undefined,
