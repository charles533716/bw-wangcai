        ...
        <el-form-item label-width="0">
          <div class="balance-card">
            <div class="balance-icon">
              <img src="@/assets/images/balance.png" alt="可用佣金" />
            </div>

            <div class="balance-right">
              <div class="balance-title">可用佣金</div>
              <div class="balance-row2">
                <span class="balance-amount">
                  {{
                    showCommission && form.agentAmount !== null && form.agentAmount !== undefined
                      ? form.agentAmount
                      : '******'
                  }}
                </span>
                <i
                  ...
                  :class="{ 'eye-close': !showCommission }"
...
<script>
import { getAgentCommission } from "@/api/agent/agentaccount"
import { addAgentCommission} from "@/api/agent/agentaccount"

export default {
  name: "AgentCommission",
  data() {
    return {
      activeTab: 'agentCommission',
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
  ...
  methods: {
    getCommissionInfo() {
      this.loading = true
      getAgentCommission(this.queryParams).then(response => {
        if (response.code === 200) {
          this.form.agentAmount = response.data.commissionAmount == null ? 0.00 : response.data.commissionAmount
        } else {
          this.form.agentAmount = 0.00
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          addAgentCommission(this.form).then(response => {
