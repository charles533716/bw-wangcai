  ...
  <div class="report-page app-container">
    ...
    <section class="report-section">
      <div class="report-heading">
        <div>场馆游戏报表表单（共 {{ filteredRows.length }} 条记录）</div>
      ...
      >
        ...
        <el-table-column prop="game" label="游戏名称" min-width="150" align="center" sortable />
        <el-table-column prop="site" label="站点" min-width="120" align="center" sortable>
          <template slot-scope="{ row }">
            <span class="risk-badge">无风险</span>
          </template>
        </el-table-column>
...
const VENUES = ['CQ9电子', 'DB哈希', 'DB棋牌', '旺财电子', 'PG电子', 'IM电竞', '熊猫体育']
const GAMES = ['欢乐捕鱼', '西游记', '隔壁王二不曾偷', '麻将胡了万倍发', 'Hilo', 'T台美女秀', '极限跳伞', '万人推筒子', '万人水果机', '超级王牌']

function createRows() {
  return Array.from({ length: 353 }, (_, index) => {
    const base = index + 1
    const profit = index % 9 === 2 ? -(base * 20 + 0.4) : (base % 7 === 0 ? -(base * 170) : base * 110 + 23)
    return {
      id: base,
      period: '2026-07-01 ~ 2026-07-29',
      bettorCount: index % 11,
      venue: VENUES[index % VENUES.length],
      game: index % 13 === 0 ? '-' : GAMES[index % GAMES.length],
      site: SITES[index % SITES.length],
      orderCount: (index % 23 + 1) * (index % 5 + 1),
      betAmount: Number(((index % 17 + 1) * 104800 + base * 12.6).toFixed(2)),
      profit: Number(profit.toFixed(2)),
      currency: index % 8 === 0 ? 'USDT' : 'CNY'
    }
  })
}
...
function defaultQuery() {
  return {
    ...
    game: '',
    currency: '',
    dateRange: ['2026-07-01 00:00:00', '2026-07-29 23:11:22']
  }
}
...
export default {
  ...
  methods: {
    ...
    exportReport() {
      this.$message.success('场馆游戏报表导出任务已生成')
    },
    refreshData() {
    ...
    summaryMethod({ columns }) {
      const totals = this.filteredRows.reduce((result, row) => {
        ...
        return result
      }, { bettorCount: 0, orderCount: 0, betAmount: 0, profit: 0 })
...
.filter-panel .el-form { display: flex; flex-wrap: wrap; align-items: center; gap: 0 20px; }
...
@media (max-width: 1280px) {
  .filter-panel .el-form { gap: 0 12px; }
  .date-filter ::v-deep .el-date-editor { width: 330px; }
}
</style>
