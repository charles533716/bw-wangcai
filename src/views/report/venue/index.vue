<template>
  <div class="app-container">
    ...
    <el-table
      ...
      size="small"
    >
      <el-table-column label="游戏平台" align="center" prop="venueName" width="120" fixed />
      ...
      <el-table-column label="投注统计" align="center" header-align="center">
        ...
        <el-table-column label="投注次数" align="center" prop="betCount" width="90" />
        <el-table-column label="投注额" align="center" prop="betAmount" width="100">
          <template slot-scope="scope">
            {{ formatCurrency(scope.row.betAmount) }}
          </template>
        </el-table-column>
        <el-table-column label="人均投注额" align="center" prop="avgBetAmount" width="100">
          <template slot-scope="scope">
            {{ formatCurrency(scope.row.avgBetAmount) }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="人均有效投注额" align="center" prop="avgValidBetAmount" width="120">
          <template slot-scope="scope">
            {{ formatCurrency(scope.row.avgValidBetAmount) }}
          </template>
        </el-table-column>
      </el-table-column>
      ...
      <el-table-column label="输赢统计" align="center" header-align="center">
        ...
        <el-table-column label="人均输赢" align="center" prop="avgWinLoss" width="100">
          <template slot-scope="scope">
            <span :class="getWinLossClass(scope.row.avgWinLoss)">
              {{ formatCurrency(scope.row.avgWinLoss) }}
            </span>
          </template>
        </el-table-column>
...
<script>
import { listGameVenueReport, exportGameVenueReport } from "@/api/report/gameVenue";
...
export default {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true;
      try {
        const response = await listGameVenueReport(this.queryParams);
    ...
    async getAllSummary() {
      ...
      try {
        const { rows } = await fetchAllRowsByPage(
          (query) => listGameVenueReport(query),
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有游戏场馆报表数据?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return exportGameVenueReport(queryParams);
      }).then(response => {
