  ...
  <div class="app-container risk-page">
    <div class="risk-card">
      ...
      >
        ...
        <el-form-item class="query-item query-item--lg" label="用户ID" prop="userId">
          <el-input
        ...
        <el-form-item class="query-item query-item--lg" label="用户名" prop="userName">
          <el-input
      ...
      <div class="risk-toolbar">
        <el-button type="primary" size="mini" icon="el-icon-plus" @click="handleAdd">新增用户白名单</el-button>
      </div>
      ...
      <el-table v-loading="listLoading" :data="listData" @row-dblclick="handleEdit">
        ...
        <el-table-column label="用户ID" align="center" min-width="140" show-overflow-tooltip>
          <template slot-scope="scope">{{ getWhitelistUserId(scope.row) }}</template>
        </el-table-column>
        <el-table-column label="用户名" align="center" min-width="170" show-overflow-tooltip>
          <template slot-scope="scope">{{ getWhitelistUserName(scope.row) }}</template>
        </el-table-column>
...
<script>
import {
  listRiskUserWhitelist,
  getRiskUserWhitelist,
  delRiskUserWhitelist,
  addRiskUserWhitelist,
  updateRiskUserWhitelist
} from '@/api/risk/userWhitelist'
...
export default {
  ...
  methods: {
    ...
    resolveWhitelistId(row) {
      return this.getFieldByAliases(row, ['id', 'whitelistId', 'userWhitelistId', 'riskUserWhitelistId'])
    },
    getWhitelistUserId(row) {
      return this.getFieldByAliases(row, ['userId', 'memberId', 'uid', 'accountId']) || '--'
    },
    getWhitelistUserName(row) {
      return this.getFieldByAliases(row, ['userName', 'username', 'name', 'memberName', 'account', 'loginName']) || '--'
    },
    ...
    normalizeForm(data = {}) {
      return {
        ...
        userId: this.getFieldByAliases(data, ['userId', 'memberId', 'uid', 'accountId']) || '',
        userName: this.getFieldByAliases(data, ['userName', 'username', 'name', 'memberName', 'account', 'loginName']) || '',
    ...
    buildPayload(source = {}, extra = {}) {
      ...
      return this.sanitizePayload({
        id: whitelistId,
        whitelistId,
        userWhitelistId: whitelistId,
        riskUserWhitelistId: whitelistId,
    ...
    buildQueryPayload() {
      const userId = String(this.queryParams.userId || '').trim()
      const userName = String(this.queryParams.userName || '').trim()
    ...
    getList() {
      ...
      listRiskUserWhitelist(this.buildQueryPayload()).then((response) => {
    ...
    handleAdd() {
      this.resetDataForm()
      this.dialogTitle = '新增用户白名单'
      this.dialogOpen = true
    },
    handleEdit(row) {
      const id = this.resolveWhitelistId(row)
      if (!id) {
        this.$modal.msgWarning('请双击一条用户白名单数据进行编辑')
        return
      }
      getRiskUserWhitelist(id).then((response) => {
        this.resetDataForm()
    ...
    handleDelete(row) {
      const id = this.resolveWhitelistId(row)
      if (!id) return
      this.$modal.confirm('确认删除当前用户白名单吗？').then(() => delRiskUserWhitelist(id)).then(() => {
    ...
    handleStatusSwitch(row) {
      ...
      this.$modal.confirm(`确认${actionText}当前用户白名单吗？`).then(() => updateRiskUserWhitelist(payload)).then(() => {
    ...
    submitForm() {
      if (!String(this.form.userId || '').trim() && !String(this.form.userName || '').trim()) {
        this.$modal.msgWarning('用户ID和用户名至少填写一项')
        return
      }
      this.$refs.formRef.validate((valid) => {
        if (!valid) return
        const payload = this.buildPayload(this.form)
        const requestFn = payload.id ? updateRiskUserWhitelist : addRiskUserWhitelist
