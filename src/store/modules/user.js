...
const user = {
  ...
  mutations: {
    ...
    SET_BACKEND_MODE: (state, backendMode) => {
      state.backendMode = backendMode
    },
    SET_IS_AGENT: (state, isAgent) => {
      state.isAgent = isAgent
    },
    SET_SITE_CODE: (state, siteCode) => {
    ...
    GetInfo({ commit, state }) {
      return new Promise((resolve, reject) => {
        getInfo().then(res => {
          ...
          commit('SET_BACKEND_MODE', res.backendMode || getCurrentBackendMode())
          commit('SET_IS_AGENT', user.isAgent || 0)
          commit('SET_SITE_CODE', user.siteCode || '')
          commit('SET_AGENT_CODE', user.agentCode || '')
    ...
    LogOut({ commit, state }) {
      return new Promise((resolve, reject) => {
        logout(state.token).then(() => {
          ...
          commit('SET_IS_AGENT', 0)
          commit('SET_SITE_CODE', '')
          commit('SET_AGENT_CODE', '')
