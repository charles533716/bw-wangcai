<template>
  <el-dialog
    ...
    width="1180px"
    append-to-body
    custom-class="advance-record-dialog"
    @close="handleClose"
  >
    <el-alert
      class="scope-tip"
...
export default {
  name: 'AgentAdvanceRecordsDialog',
  props: {
    visible: { type: Boolean, default: false },
    scope: { type: String, default: 'master' },
  ...
  computed: {
    effectiveSiteCode() {
      return this.siteCode || this.$store.getters.userSiteCode || DEFAULT_SITE_CODE
    },
    effectiveAgentCode() {
      return this.agentCode || this.$store.getters.agentCode || DEFAULT_AGENT_CODE
    },
    scopeTip() {
      if (this.scope === 'agent') {
        return '当前仅展示本代理的佣金预支记录，可用于核对领取结果和剩余次数。'
      }
      if (this.scope === 'site') {
        return `当前仅展示${this.siteName || '当前站点'}下代理的佣金预支记录，规则沿用总控配置。`
      }
